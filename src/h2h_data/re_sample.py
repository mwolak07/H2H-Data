...
def re_sample(data: List[Tuple[float, Any]], method: Optional[SamplingMethod] = SamplingMethod.TO_TIMESTAMPS,
    ...
    if method == SamplingMethod.TO_TIMESTAMPS:
        return re_sample_to_timestamps(data, args['timestamps'])
    if method == SamplingMethod.SIMPLE:
        return re_sample_simple(data, args['source_fps'], args['target_fps'])
    else:
        raise ValueError(f'Method {method} is not supported.')


def re_sample_to_timestamps(data: List[Tuple[float, Any]], timestamps: List[float]) -> List[Tuple[float, Any]]:
    """
...
def re_sample_simple(data: List[Tuple[float, Any]], source_fps: int, target_fps: int) -> List[Tuple[float, Any]]:
    """
